...
class UpstageChat: #KoreaVisaRAG 에서 인스턴스 생성 및 호출함
    ...
    def generate_answer(self, current_question: str, user_profile: dict, chat_history: list, context_chunks: list):
        ...
        prompt = f"""당신은 한국 비자 및 체류 관련 전문 상담사입니다. 아래 문서들을 참고하여 사용자의 질문에 정확하고 도움이 되는 답변을 제공해주세요.
                    ...
                    {retrieved_docs_context}
                    ---
                    위의 **사용자 정보, 이전 대화 내용, 참고 자료**를 모두 종합하여 아래 사용자의 질문에 대해 가장 정확하고 개인화된 답변을 제공해주세요.
                    ...
                    0. 만약 사용자에게서 들어온 질문의 내용이 "예/응" 등이라면 대화 기록에서 사용자가 가장 최근에 한 질문에 대해 사용자가 가장 최근에 한 답변을 위주로 대답하세요.

                    0. 만약 사용자에게서 들어온 질문의 내용이 마치 어떤 질문이 아닌 잘문에 대한 "답변" 같다면 다음과 같이 답변하세요 "넵 정보를 파악하였습니다" 질문을 계속 하시겠어요? 희망하시면 "예 / 응" 등으로 대답해주세요!
                    
                    1. 만약 추출된 청크들 중 이 정보가 이 사용자한테 전달되기 적절한 정보인지 알기 위해 추가적으로 사용자에게 물어보고 싶은 데이터가 있다면 물어봐도 됩니다.
