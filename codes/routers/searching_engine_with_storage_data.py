...
class UpstageChat: #KoreaVisaRAG 에서 인스턴스 생성 및 호출함
    ...
    def  make_final_return_value(self , summarized_info: str , retrieved_docs_context , current_question):
          ...
          prompt_last = f"""당신은 한국 비자 및 체류 관련 전문 상담사입니다. 앞의 ai 가 사용자와 ai 가 주고 받은 대화를 바탕으로 정리한 사용자의 정보를 바탕으로 참고하여 사용자의 질문에 정확하고 도움이 되는 답변을 제공해주세요.
                    ...
                    0. 반드시!!!!!!!!  질문으로 들어온 언어로 답변하세요!!
                    
                    1. 만약 추출된 청크들 중 이 정보가 이 사용자한테 전달되기 적절한 정보인지 알기 위해 추가적으로 사용자에게 물어보고 싶은 데이터가 있다면 얼마든지 최대한 많이 물어보세요
                    ...
                    5. 모르는 내용은 추측하지 말고 "제공된 정보로는 확인이 어렵습니다"라고 답하고 , 참고 자료에서 사용자에게 도움이 될 것 같은 링크를 첨부하세요 
                          예를 들어 , 사용자의 질문에 대한 명확한 정보를 제공하긴 어렵지만 대신 관련 정보가 있을 것으로 추정되는 링크를 드릴게요!
                    ...
                    10. 반드시!!!!!!!!  질문으로 들어온 언어로 답변하세요!!

                    11. 다시 한번 조건 1~10을 순회하며 지금 만들어진 답변이 무시한 제약조건이 없는지 확인하세요 
                    12. 지금 전달된 프롬프트에 '예시'로 작성된 문장을 아무 근거 없이 똑같이 쓰진 않았는지 확인하세요!
